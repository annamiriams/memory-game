...
const matchFoundMsg = document.querySelector('#match-success');
// used to iterate each correct match
const matchesTallyMsg = document.querySelector('#matches-tally');
// used to iterated each turn taken
const turnsTallyMsg = document.querySelector('#turns-tally');
...
const flipCard = function() {    
    // issue: i can click many cards in succession and checkForMatch seems to get all screwy. it seems like creating the variable freezeBoard will help: at round start, freezeBoard is false (therefore board is not frozen). we need to freeze the board after the second click to allow time for checkForMatch to run, and then once checkForMatch is complete (which really means when flipCardsBack is complete since that's at the end of checkForMatch), we can set freezeBoard back to false.
    ...
    // return used here means that when flipCard is run, and if freezeBoard === true, the rest of the function will not run. 
    if (freezeBoard === true) {
        return;
    }
...
const checkForMatch = function() {
    if (firstCard.id === secondCard.id) {
        ...
        matchFoundMsg.textContent = `You found two ${firstCard.id}s!`
        incMatchesTally();
        compare();
    }
    else {
...
const incTurnsTally = function () {
    turnsTally++;
    turnsTallyMsg.textContent = `${turnsTally} / ${turnsAvailable} turns`
};
...
const incMatchesTally = function () {
    matchesTally++;
    matchesTallyMsg.textContent = `${matchesTally} / ${matchesAvailable} matches`
};
